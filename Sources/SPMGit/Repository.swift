...
final public class Repository {
    ...
    public func lookupTag(_ objectID: ObjectID) throws -> Tag {
        ...
        print("\(handle): lookupTag(\(objectID))")
    ...
    public func getTags() throws -> [Tag] {
        ...
        /// Context object used during iteration.
        struct Context {
        ...
        /// Create an empty context and get access to its pointer
        var context = Context(repository: self, tags: [], error: nil)
        _ = try withUnsafeMutablePointer(to: &context) { context in

            /// Iterate over all tags, passing our context as payload.
            try validate(git_tag_foreach(handle, { (name, oid, context) in

                /// Convert arguments into Swift types.
                let name = String(cString: name!)
                let objectID = ObjectID(oid: oid!.pointee)
                let context = context!.assumingMemoryBound(to: Context.self)
                print("\(context.pointee.repository.handle): \(name): \(objectID)")

                do {
                    /// Lookup the tag object corresponding to this identifier.
//                    let tag = try context.pointee.repository.lookupTag(objectID)
//                    context.pointee.tags.append(tag)
                    var tagHandle: Tag.Handle? = nil
                    try validate(git_tag_lookup(&tagHandle, context.pointee.repository.handle, oid))
                    context.pointee.tags.append(Tag(handle: tagHandle!))
                    return 0
                } catch {
                    /// On error, record the error and bail out.
        ...
        /// Report errors that happened during iteration.
